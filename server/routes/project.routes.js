...
router.get('/myProjects', (req,res) => {
  ...
  const PromisesYeah = [
    ProjectTrends.find({creatorId: req.user._id}).then(r=> r),
    ProjectWords.find({creatorId: req.user._id}).then(r=> r),
    ProjectUser.find({creatorId: req.user._id}).then(r=> r)
  ]
  Promise.all(PromisesYeah).then(x => res.json(x))
...
router.get('/getAllProjects', (req, res) => {
    ...
    const Promises = [
      ProjectTrends.find().then(r=> r),
      ProjectWords.find().then(r=> r),
      ProjectUser.find().then(r=> r)
    ]
    Promise.all(Promises).then(x => res.json(x))
    .catch(err => console.log({err}))
})

router.get('/getOneProject/:id', (req, res) => {
    const Promises = [
      ProjectTrends.findById(req.params.id).then(r => r),
      ProjectUser.findById(req.params.id).then(r => r),
      ProjectWords.findById(req.params.id).then(r => r)
    ]

    Promise.all(Promises).then(theProject => res.json(theProject))
        .catch(err => console.log('Error', err))
})

router.post('/likeOneProject', (req,res) => {
  console.log(req.body)
  const Promises = [
    ProjectTrends.findByIdAndUpdate(req.body._id, {$set:{likes: req.body.likes + 1 }},{new: true} ).then(r => r),
    ProjectUser.findByIdAndUpdate(req.body._id, {$set:{likes: req.body.likes + 1 }},{new: true} ).then(r => r),
    ProjectWords.findByIdAndUpdate(req.body._id, {$set:{likes: req.body.likes + 1 }},{new: true} ).then(r => r)
  ]

  Promise.all(Promises).then(theProject => {
    console.log(theProject)
    res.json(theProject)})
      .catch(err => console.log('Error', err))
})

router.post('/commentOneProject', (req,res) => {
  console.log(req.body)
  const Promises = [
    ProjectTrends.findByIdAndUpdate(req.body.project._id, {$push:{coments: req.body.comment }},{new: true} ).then(r => r),
    ProjectUser.findByIdAndUpdate(req.body.project._id, {$push:{coments: req.body.comment }},{new: true} ).then(r => r),
    ProjectWords.findByIdAndUpdate(req.body.project._id, {$push:{coments: req.body.comment }},{new: true} ).then(r => r)
...
router.post('/postProject', (req, res) => {
  ...
  router.post('/postProject/Trends', (req, res) => {
    ...
    .then(() => {
      ...
      .then(theNewProject => {
        User.findByIdAndUpdate(req.body.creatorId, {$push: {posts: theNewProject._id}}, {new: true}).then(x=>console.log("Teo arreglo esto en un minuto <3"))
        res.json(theNewProject)
      })
    })
    .catch(err => console.log('Error', err))
...
router.post('/postProject/User', (req, res) => {
  ...
  .then(tweets => {
    ...
    .then(resulon => res.json(resulon))
  })
  .catch(err => console.log('Error', err))
})

router.post('/updateProject/Words', (req, res) => {
  const {title, placesArray} = req.body
   ProjectWords.findOneAndUpdate({title: title}, {$set: {placesArray}}, {new: true})
  .then(x => console.log(x))
  .catch(err => console.log(err))
})
